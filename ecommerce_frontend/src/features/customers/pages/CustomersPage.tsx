...
export default function CustomersPage() {
  ...
  const { data, isPending, isFetching } = useCustomers({ page, perPage: 10, search: debouncedSearch });
  ...
  const header = useMemo(() => `${pagination?.total ?? 0} عميل`, [pagination?.total]);
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-slate-600">
              <tr>
                ...
                <th className="text-left p-3">الاسم</th>
                <th className="text-left p-3">الهاتف</th>
                <th className="text-left p-3">الدولة</th>
              ...
              {isPending && !data ? (
                ...
                customers.map((c) => (
                  <tr key={c.id} className="border-t hover:bg-slate-50 transition">
                    ...
                    <td className="p-3 text-slate-600 line-clamp-2 max-w-[360px]">{c.address}</td>
